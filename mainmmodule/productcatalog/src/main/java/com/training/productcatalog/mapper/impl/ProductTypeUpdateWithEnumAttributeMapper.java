...
public class ProductTypeUpdateWithEnumAttributeMapper extends ProductTypeAbstractMapper<ProductType, ProductTypeUpdateCommand, ProductTypeRequest> {
    ...
    public ProductTypeUpdateCommand map(ProductTypeRequest request) {
        ...
        final List<EnumValue> enumValues = request.getAttributeEnumValues().stream()
                .map(value -> EnumValue.of(value, value)).collect(Collectors.toList());
        final AttributeDefinition attributeDefinition = AttributeDefinitionBuilder.of(attributeName,
                ...
                EnumAttributeType.of(enumValues)).build();
